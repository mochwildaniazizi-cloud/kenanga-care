...
const CHILDREN_DATA = [
  ...
  // Lilis Suryani (hamil, punya 1 anak sebelumnya)
  ...
  // Sri Wahyuni (hamil, 3 anak sebelumnya)
...
export async function GET() {
  ...
  try {
    ...
    for (const m of MOTHERS) {
      ...
      statusCount[m.ui_status] = (statusCount[m.ui_status] ?? 0) + 1;
      log.push(`✅ Ibu [${m.ui_status}]: ${m.mother_name}`);
    }
    ...
    for (const c of CHILDREN_DATA) {
      ...
      if (!motherId) {
        log.push(`⚠️ Ibu tidak ditemukan: ${c.child_name}`);
        continue;
      }
      ...
      childCount++;
      log.push(`👶 Anak [${childCount.toString().padStart(2, "0")}]: ${c.child_name}`);
    }
